...
CBlockTemplate* CreateNewBlock(const CScript& scriptPubKeyIn, CWallet* pwallet, bool fProofOfStake)
{
    ...
    if(prev->nHeight <= 9) { txNew.vout[0].nValue = 100000 * COIN;} // Premine 100,000 * 10 = 1,000,000 4% premine
    if (prev->nHeight > 9 && prev->nHeight <= 300) { txNew.vout[0].nValue = 2 * COIN;} // Block 9 to 300 pay 1 to prevent instamine
    if(prev->nHeight > 300) { txNew.vout[0].nValue = 20 * COIN;} // Block 300 or higher pay 20
...
bool ProcessBlockFound(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != chainActive.Tip()->GetBlockHash())
            return error("ltclMiner : generated block is stale");
    }
    ...
    if (!ProcessNewBlock(state, NULL, pblock))
        return error("ltclMiner : ProcessNewBlock, block not accepted");
...
{
    LogPrintf("ltclMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("ltcl-miner");
    ...
    while (fGenerateBitcoins || fProofOfStake) {
        ...
        LogPrintf("Running ltclMiner with %u transactions in block (%u bytes)\n", pblock->vtx.size(),
