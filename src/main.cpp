...
#if defined(NDEBUG)
#error "ltcl cannot be compiled without assertions."
#endif
...
//Premine 1.5%   112,500 * 10 blocks = 1,125,000
//from block 10 to 300 block reward will be 1 to prevent instamine
//from block 301 or higher block reward 140
	if (nHeight <= 9) {
        nSubsidy = 100000 * COIN;
   	 } 
        else if (nHeight > 9 && nHeight <= 300 ) {
        nSubsidy = 2 * COIN;   
     	} else if (nHeight <= 25920 && nHeight > 300) {
        nSubsidy = 20 * COIN;
        } else if (nHeight >= 25921) {
        nSubsidy = 25 * COIN;
         }
return nSubsidy;
}
