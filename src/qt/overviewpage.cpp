...
void OverviewPage::updateObfuscationProgress()
{
    ...
    QString strAnonymizeltclAmount = BitcoinUnits::formatHtmlWithUnit(nDisplayUnit, nAnonymizeltclAmount * COIN, false, BitcoinUnits::separatorAlways);
    ...
    if (currentBalance == 0) {
        ...
        strAnonymizeltclAmount = strAnonymizeltclAmount.remove(strAnonymizeltclAmount.indexOf("."), BitcoinUnits::decimals(nDisplayUnit) + 1);
        strAmountAndRounds = strAnonymizeltclAmount + " / " + tr("%n Rounds", "", nObfuscationRounds);
    ...
    if (nMaxToAnonymize > nAnonymizeltclAmount * COIN) nMaxToAnonymize = nAnonymizeltclAmount * COIN;

    if (nMaxToAnonymize == 0) return;

    if (nMaxToAnonymize >= nAnonymizeltclAmount * COIN) {
        ui->labelAmountRounds->setToolTip(tr("Found enough compatible inputs to anonymize %1")
                                              .arg(strAnonymizeltclAmount));
        strAnonymizeltclAmount = strAnonymizeltclAmount.remove(strAnonymizeltclAmount.indexOf("."), BitcoinUnits::decimals(nDisplayUnit) + 1);
        strAmountAndRounds = strAnonymizeltclAmount + " / " + tr("%n Rounds", "", nObfuscationRounds);
    } else {
