...
void setupAddressWidget(QValidatedLineEdit* widget, QWidget* parent)
{
    ...
    widget->setPlaceholderText(QObject::tr("Enter a ltcl address (e.g. %1)").arg("D7VFR83SQbiezrW72hjcWJtcfip5krte2Z"));
#endif
...
{
    // return if URI is not valid or is no ltcl: URI
...
{
    // Convert ltcl:// to ltcl:
    //
    //    Cannot handle this later, because ltcl:// will cause Qt to see the part after // as host,
...
void openConfigfile()
{
    ...
    /* Open ltcl.conf with the associated application */
...
{
    return GetSpecialFolderPath(CSIDL_STARTUP) / "ltcl.lnk";
}
...
{
    return GetAutostartDir() / "ltcl.desktop";
}
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    if (!fAutoStart)
        ...
        if (!optionFile.good())
            return false;
        // Write a ltcl.desktop file to the autostart directory:
        optionFile << "[Desktop Entry]\n";
        optionFile << "Type=Application\n";
        optionFile << "Name=ltcl\n";
...
LSSharedFileListItemRef findStartupItemInList(LSSharedFileListRef list, CFURLRef findUrl)
{
    // loop through the list of startup items and try to find the ltcl app
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    ...
    if (fAutoStart && !foundItem) {
        // add ltcl app to startup item list
